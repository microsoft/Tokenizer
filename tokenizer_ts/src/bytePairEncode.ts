...
*/
export function bytePairEncode(mergingBytes: Uint8Array, ranks: ReadonlyMap<string, number>): number[] {
    ...
    function getRank(startIndex: number, skip = 0): number {
      if (startIndex + skip + 2 < byteIndicesAndRanks.length) {
        const slice = mergingBytes.slice(byteIndicesAndRanks[startIndex][0], byteIndicesAndRanks[startIndex + skip + 2][0]);
    ...
    for (let i = 0; i < byteIndicesAndRanks.length - 1; i++) {
      outList.push(ranks.get(uint8ArrayToString(mergingBytes.slice(byteIndicesAndRanks[i][0], byteIndicesAndRanks[i + 1][0])))!);
    }
    return outList;
  }