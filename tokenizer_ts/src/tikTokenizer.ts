...
export class TikTokenizer {
     ...
     */
    constructor(tikTokenBpeFile: string, specialTokensEncoder: ReadonlyMap<string, number>, regexPattern: string, cacheSize: number = 8192) {
    ...
    private findNextSpecialToken(text: string, start: number, allowedSpecial?: ReadonlyArray<string>): [RegExpMatchArray | null, number] {
        let startFind = start;
        ...
        if (allowedSpecial && this.specialTokensRegex)
        {
            while (true) {
                ...
                if (allowedSpecial && allowedSpecial.includes(nextSpecial[0])) break;
     ...
     */
    public encode(text: string, allowedSpecial?: ReadonlyArray<string>): number[] {
    ...
    private encodeSpecialToken(tokenIds: number[], nextSpecial: RegExpMatchArray): number {
        const token = this.specialTokensEncoder?.get(nextSpecial[0]);
        tokenIds.push(token!);
        return nextSpecial.index! + nextSpecial[0].length;
    }

    private encodeByIndex(text: string, tokenIds: number[], start: number, end: number): void {
    ...
    private encodeTrimSuffixByIndex(text: string, tokenIds: number[], start: number, end: number, maxTokenCount: number, tokenCount: number, encodeLength: number): { tokenCount: number, encodeLength: number } {
        let match: RegExpExecArray | null | undefined;
     ...
     */
    public encodeTrimSuffix(text: string, maxTokenCount: number, allowedSpecial: ReadonlyArray<string>): { tokenIds: number[], text: string } {
        const tokenIds: number[] = [];
     ...
     * @returns { tokenIds: number[], text: string } encoded token ids and trimmed text
     */
    public encodeTrimPrefix(text: string, maxTokenCount: number, allowedSpecial?: ReadonlyArray<string>): { tokenIds: number[], text: string } {
        const tokenIds: number[] = [];
        ...
        return { tokenIds: tokenIds.slice(actualPrefixTokenCount), text: text.slice(actualPrefixStrLength) };
    }
