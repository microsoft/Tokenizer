...
suite('TikTokenizer Test Suite', function () {
  ...
  test('encode trim suffix', () => {
    ...
    let encoded = tokenizer.encodeTrimSuffix(str, 4, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimSuffix(str, 5, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimSuffix(str, 3, Array.from(specialTokens.keys()));
  ...
  test('encode trim suffix - 2', () => {
    ...
    let encoded = tokenizer.encodeTrimSuffix(str, 5, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 20539, 10343, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimSuffix(str, 6, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 20539, 10343, 100265]);
    assert.deepStrictEqual(encoded.text, str);
  ...
  test('encode trim prefix', () => {
    ...
    let encoded = tokenizer.encodeTrimPrefix(str, 4, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimPrefix(str, 5, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimPrefix(str, 3, Array.from(specialTokens.keys()));
  ...
  test('encode trim prefix - 2', () => {
    ...
    let encoded = tokenizer.encodeTrimPrefix(str, 5, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 12427, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);

    encoded = tokenizer.encodeTrimPrefix(str, 6, Array.from(specialTokens.keys()));
    assert.deepStrictEqual(encoded.tokenIds, [100264, 9906, 12427, 4435, 100265]);
    assert.deepStrictEqual(encoded.text, str);
  ...
  test('tokenize source code - gpt-3.5', () => {
